...
void Test::test_time(uint32_t digit, int iterations)
{
    RSA rsa;
    RSA::find_cnts = 0;
    RSA::miller_cnts = 0;
    ...
    for (int i = 0; i < iterations; i++) {
        time_used += rsa.set_up(digit);
    }
    cout << "Test done, digit = " << digit << ", average time = " << time_used / iterations << endl;
    cout << "Find cnt:" << (double)RSA::find_cnts/(iterations*2) << endl;
    cout << "Miller cnt:" << (double)RSA::miller_cnts/(iterations*2) << endl;
}
